...
> **Current focus (2026):** the active work has moved beyond sports. See **[`research/`](research/README.md)** for the multi-market **prediction lab** (MLB/NHL/soccer win-prob models, DeFi time-series, crypto volatility forecasting, funding carry) and **[`lob-engine/`](lob-engine/README.md)** for a native-Rust **trading-simulator game with an AI strategy coach** — replay a real order-book tape, trade against the reconstructed book, and ask an embedded AI coach for advice (`cargo run -p tui -- <tape>`); under the hood a genuine low-latency market-data / order-book / replay engine. The sports pipeline documented below remains intact as the original scaffolding.
