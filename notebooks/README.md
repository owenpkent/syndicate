...
| `14_crypto_movement_sim.ipynb` | `data/defi.duckdb` | Predicting *movement* — magnitude not direction. Reuses `research/defi/vol_forecast_sim.py` (walk-forward HAR / HAR-CJ realized-vol forecast vs naive/EWMA: OOS R²≈0.4, calibrated slope≈1, beats baselines; jumps help marginally) + a vol-targeting gated backtest (stabilizes risk, no alpha), and `leadlag_candles.py` (HL perp vs Coinbase spot: synced at lag 0, but spot mildly *leads* the perp — asymmetric ΔR², strongest in thin XRP). |
